...
| 38 | Mint `acif.registry.reference_unresolved` (ratifies shipped behavior; resolves syllago Stage 2 Gap 2) | PROTOCOL §4.11 required an unresolved reference to carry a §3.1 diagnostic with `params.declared_name`, and [ACIF-REGISTRY] §9 made the entry-level `diagnostic` field REQUIRED for `unresolved \| revoked` — but no spec minted an identifier for the condition ([ACIF-REGISTRY] §12 had no reference-resolution row, and §3.1 diagnostics obey the mint discipline: "this table never mints"). A conforming adapter had no legal id to emit; both the reference adapter and syllago (impl #1, Stage 2, 2026-07-13) had independently shipped `acif.registry.reference_unresolved` unminted. **Resolution (ratifies shipped behavior):** mint `acif.registry.reference_unresolved`, class `diagnostic (MUST-emit)`, [ACIF-REGISTRY] §12 — emitted on the computing response once per reference whose resolution lands `unresolved` or `revoked`, `params` carrying `declared_name` (the declared reference string as written: the name for name-declared refs, the UUID for UUID-authored ones). The §9 entry-level `diagnostic` field carries the same identifier (was free-text; the missing target is already named by the entry's `declared_name`/`source_path`). Payload-pinned in PROTOCOL Appendix A (`declared_name`, asserting vector TV-AGENT-j). **Rejected:** a separate `reference_revoked` id — the entry's `resolution` field already discriminates the states, no vector asserts revoked separately, and a second identifier adds vocabulary without a consumer (roadmap if a revoked-specific policy lane ever needs it); leaving the id unminted per-implementation (the two shipped implementations converged by luck, impl #3 could legally diverge). **Vector-impact inventory: TV-AGENT-j** — state_2 previously passed on ANY diagnostic carrying `params.declared_name`; it now requires the minted id (expect restructured to the TV-RULE-m `diagnostic` + `diagnostic_names` shape). Syllago already emits the minted id — zero syllago-side changes. No other vector touches reference resolution diagnostics (TV-SKILL-j resolves cleanly). Companion consistency fixes, same landing: [ACIF-REGISTRY] §9 `target_id` REQUIRED scoped to name-declared references (UUID-authored refs MAY omit it — the declared identifier is the target id; aligns §9 with published TV-SKILL-j/TV-AGENT-j expectations under the vectors-win rule); `acif.rule.activation_degraded` moved from Appendix A's identifier-only list to the payload-pinned table (`mode_lost`, `effective_behavior` — TV-RULE-m had asserted its params all along); PROTOCOL §3.1's claimed Appendix-A coverage self-check implemented (it had been prose-only — now reconciles payload-asserting bindings against the table in both directions). No reviewer subset: like Decision #37, this ratifies shipped, suite-implied behavior — the protocol already required the diagnostic and pinned its params key; only the identifier value was missing. |
